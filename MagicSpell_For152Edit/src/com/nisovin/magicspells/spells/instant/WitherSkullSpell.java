...
public class WitherSkullSpell extends InstantSpell implements TargetedEntityFromLocationSpell {
	...
	public PostCastAction castSpell(Player player, SpellCastState state, float power, String[] args) {
		if (state == SpellCastState.NORMAL) {
			...
			skull.setDirection(player.getLocation().getDirection().multiply(velocity * power));
		}
	...
	public boolean castAtEntityFromLocation(Player caster, Location from, LivingEntity target, float power) {
		...
		Location ff = from.clone();
		WitherSkull skull = from.getWorld().spawn(ff, WitherSkull.class);
		skull.setVelocity(v);
		skull.setVelocity(v);
